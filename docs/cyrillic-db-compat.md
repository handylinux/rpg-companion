...
## 8. `components/CharacterContext.js`

### 8.1 Кириллический ключ `Название` в `getItemId`
```js
return item.weaponId || item.code || item.Name || item.Название;
```
**Причина:** в старых сохранениях/данных снаряжения встречается русскоязычный ключ `Название`.
Этот fallback нужен для стабильного определения ID модифицированного предмета.
**Что сделать при миграции БД:** убрать `item.Название`, оставить только латинские поля идентификатора.
